...
export function printPlan(): void {
  ...
  let h = `<h1 style="font-size:16pt;margin-bottom:2pt">🦆 Task Duck — Working Checklist</h1>`;
  ...
  if (state.lastVerdict && state.lastVerdict !== 'match' && ($('driftReason') as HTMLInputElement).value) {
    h += `<h3 style="font-size:11pt;margin:6pt 0 3pt;color:#c00">⚠ Known Drift</h3><p style="font-size:10pt">${esc(($('driftReason') as HTMLInputElement).value)}</p>`;
  }
...
export function printFinal(): void {
  let h = `<h1 style="font-size:16pt;margin-bottom:2pt">🦆 Task Duck — Final Report</h1>`;
  ...
  state.diffItems.forEach(d => { h += `<div style="font-size:10pt;margin:2pt 0">${d.status === 'done' ? '✅' : '❌'} ${esc(d.text)}</div>`; });
  if (state.extras.length) { h += `<h4 style="margin:4pt 0;color:#c00">Extras</h4>`; state.extras.forEach(e => { h += `<div style="font-size:10pt;margin:2pt 0">⚠ ${esc(e.text)}</div>`; }); }
  if (state.amendments.length) { h += `<h4 style="margin:4pt 0;color:#36f">Amendments</h4>`; state.amendments.forEach(a => { h += `<div style="font-size:10pt;margin:2pt 0">📋 ${esc(a.text)}</div>`; }); }
...
export function copyShipSummary(): void {
  ...
  navigator.clipboard.writeText(md).then(() => {
    ...
    duckSay("Copy failed — try manually.");
  });
}
